...
internal fun DesktopProtectionCard(
    ...
    Surface(
        ...
        color = if (!enabled || granted) MaterialTheme.colorScheme.surface else MaterialTheme.colorScheme.secondaryContainer
    ) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text(
                when {
                    !enabled -> "关闭后不检查当前前台应用，自动和手动更换均可直接执行。"
                    granted -> "游戏、其他应用、锁屏或无法确认桌面时均跳过更换，自动任务在下个检查周期再尝试。"
            ...
            if (enabled && settingsError.isNotBlank()) Text(settingsError, style = MaterialTheme.typography.bodySmall)
        }
    }
}
